// src/components/ui/auth/PhoneInput.tsx
'use client'

import { motion, AnimatePresence } from 'framer-motion'
import { useValidatePhone, formatPhoneMX } from '@/hooks/useValidatePhone'
...
export function PhoneInput({
  ...
  const validation = useValidatePhone(value, validateDuplicate)
  ...
  const showSuccess = validation.isValid && value.length === 10 && !validation.isChecking && validation.message
  const showError = !validation.isValid && value.length > 0 && !validation.isChecking
  const showLoading = validation.isChecking

  // Mostrar con formato MX
  const displayValue = formatPhoneMX(value)
